...
self.addEventListener('install',event=>{
    ...
    const cacheProm = caches.open('estatico-v2')  //cache-1
    .then(cache=>{
        return cache.addAll([
            '/',
            '/index.html',
        ...
        const falloUnaVez=()=>{
            if(rechazada){
                if(/\.(png|jpg)$/i.test(event.request.url)){
                    resolve(caches.match('/img/no-img.png'));
                }else{
                    reject('No se encontro la respuesta')
                }
...
// //       si no existe el recurso, tenemos que conectarnos a internet z
